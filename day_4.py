...
def valid_passports(passports_file: str) -> int:
    passports = []
    with open(passports_file) as fh:
        ...
        for index, line in enumerate(contents, start=1):
            ...
            if line:
                ...
                append = True
            if append or index == length_of_contents:
                passports.append(passport.strip())
                passport = ""  # start over for next passport
    print(passports)
...
def valid_passports_2(passports_file: str) -> int:
    passports = []
    with open(passports_file) as fh:
        contents = fh.read().splitlines()
        passport = ""
        ...
        for index, line in enumerate(contents, start=1):
            ...
            if append or index == length_of_contents:
                ...
                passport = ""  # start over for next passport
    print(passports)
    print(f"Length of passprts is : {len(passports)}")
    required_fields = {"byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"}
    valid = 0
    accepted_passports = []
    ...
    for p in passports:
        ...
        fields = set()
        for combination in values:
            field, value = combination.split(":")
            field = field.strip()
            value = value.strip()
            if field == "byr":
                if not re.match("^\d{4}", value):
                    print(f" field {field} value {value} failed")
                    break  # This passport is invalid no need to do further checks
                byr = int(re.match("^\d{4}$", value).group())
        ...
        if fields == required_fields:
            valid += 1
            print(f"passport {p} is valid")
        else:
            print(f"passport {p} is invalid")
        print("---" * 40)

    return valid


# Put this in a file and pass it to the functions above
input_data = """
